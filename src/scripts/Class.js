const MOCKAPI_APPTS_URL =
  "https://68e85a87f2707e6128ca934a.mockapi.io/crearCita";
...
function renderAppts() {
  ...
  if (!Array.isArray(appointments) || appointments.length === 0) {
    $appointments.innerHTML =
      '<div class="p-4 bg-yellow-50 rounded">No hay citas programadas.</div>';
    return;
  }
  ...
  for (const a of appointments) {
    ...
    node.querySelector(".appt-meta").textContent = `${a.date || ""} · ${
      a.time || ""
    } · ${a.service || ""}`;
...
async function loadAppts() {
  if (!MOCKAPI_APPTS_URL || MOCKAPI_APPTS_URL.trim() === "") {
    $appointments.innerHTML =
      '<div class="p-4 bg-yellow-50 rounded">Configura MOCKAPI_APPTS_URL en app.js para cargar citas.</div>';
    return;
  }
  try {
...
async function updateAppt(id, payload) {
  const res = await fetch(`${MOCKAPI_APPTS_URL}/${id}`, {
    method: "PUT",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify(payload),
  });
...
apptForm.addEventListener("submit", async (e) => {
  ...
  try {
    if (!MOCKAPI_APPTS_URL || MOCKAPI_APPTS_URL.trim() === "")
      throw new Error("MOCKAPI_APPTS_URL no configurada");
    if (id && id.trim() !== "") {
